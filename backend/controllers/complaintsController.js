...
const complaintsController = {
        ...
        try {
            ...
            if (image) {
                ...
                if (!image.startsWith("data:image")) {
                    throw new Error("Invalid image format. Ensure it is a Base64-encoded image with a proper MIME type.");
                }
            ...
            res.status(201).json({ id: response.id, message: "Complaint created successfully" });
    ...
    getAllComplaints:async (req, res) => {
        ...
        try{
            const response = await db.collection('complaints').where('userId', '==', userId).orderBy("date", "desc").get();
            if(!response.empty){
                res.status(200).json({data: response.docs.map((doc) => ({id: doc.id, ...doc.data()}))});
            }else{
    ...
    updateRatingById: async (req, res) => {
        ...
        try {
            const complaintQuery = await db.collection("complaints").doc(complaintId).get();
            if (!complaintQuery.exists) {
                return res.status(204).json({ message: "Complaint not found" });
            }

            await db.collection("complaints").doc(complaintId).set({ rating }, { merge: true });
    ...
    updateStatusById: async (req, res) => {
        ...
        try {
            ...
            await db.collection("complaints").doc(complaintId).set({ status }, { merge: true });
            res.status(200).json({ message: "Complaint updated successfully" });
        } catch (error) {
            console.error("Error updating complaint:", error);
            res.status(500).json({ error: error.message });
