...
const meritsController = {
    ...
    getMeritByUserId: async (req, res) => {
        ...
        try {
            const response = await db.collection('merits').where('userId', '==', userId).get();
            if (response.empty) {
    ...
    getAllMerits: async (req, res) => {
        try{
            const response = await db.collection("merits").orderBy("totalMerits", "desc").get();
            const merits = [];
    ...
    createMerit: async (req, res) => {
        ...
        try {
            const response = await db.collection("merits").add({ userId, events, totalMerits, ranking });
            res.status(200).json({ message: "Merit created successfully", id: response.id });
        } catch (error) {
    ...
    updateMeritByUserId: async (req, res) => {
        ...
        try {
            const meritQuery = await db.collection("merits").where("userId", "==", userId).get();
            if (meritQuery.empty) {
                return res.status(204).json({ message: "Merit not found" });
            }
            const meritDoc = meritQuery.docs[0];

            await db.collection("merits").doc(meritDoc.id).update({ events, totalMerits, ranking });
