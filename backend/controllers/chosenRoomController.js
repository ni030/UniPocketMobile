...
const chosenRoomController = {
    ...
    createChosenRoom: async (req, res) => {
        ...
        try {
            const response = await db.collection("chosenRooms").add({ userId, block, roomNum });
            res.status(200).json({ message: "Chosen room created successfully", id: response.id });
        } catch (error) {
    ...
    getChosenRoomByUserId: async (req, res) => {
        ...
        try {
            const response = await db.collection('chosenRooms').where('userId', '==', userId).get();
            if (response.empty) {
    ...
    updateChosenRoomByUserId: async (req, res) => {
        ...
        try {
            const chosenRoomQuery = await db.collection("chosenRooms").where("userId", "==", userId).get();
            ...
            await db.collection("chosenRooms").doc(chosenRoomId).update({ block, roomNum });
